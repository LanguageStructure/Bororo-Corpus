...
def conllu_relations(path):
 """Extrai relações documentadas no nível da palavra; não segmenta morfemas."""
 ...
 with path.open(encoding='utf-8') as f:
  for line in f:
   ...
   form,lemma,upos,xpos=cols[1],cols[2],cols[3],cols[4];md=misc_dict(cols[9])
   ortho=md.get('ORTHO') or form
 ...
 for a in acc.values():
  ...
  for field in ['lemmas','upos','pos_detalhada','glossas','classes_posse','procliticos']:
   row[field]=[{'value':v,'frequency':n} for v,n in a[field].most_common()]
  out.append(row)
 return sorted(out,key=lambda x:(-x['frequency'],x['form'].casefold()))
def morphology_data():
 lexical=conllu_relations(CONLLU)
 editorial=[]
 if MORPH.exists():
  mr=tsv(MORPH);require(mr,['form','segmentation','morphemes','gloss','status','note'],MORPH)
  for r in mr:
   editorial.append({k:(r.get(k) or '').strip() for k in ['form','segmentation','morphemes','gloss','status','note']})
...
def main():
 ...
 md=morphology_data();(OUT/'morphology.json').write_text(json.dumps(md,ensure_ascii=False,separators=(',',':')),encoding='utf-8')
 print(f"Geradas {len(allu)} unidades ({len(coq)} Coqueiro, {len(hm)} História Mítica; {stats['provisional_units']} provisórias) e {len(md['relacoes_lexicais'])} formas com relações do CoNLL-U.")
