...
COQ=ROOT/'CorBo_vNext/texts/coqueiro/coqueiro_parallel.tsv'; HM=ROOT/'CorBo_vNext/texts/historia-mitica/historia_mitica_collation.tsv'; MORPH=ROOT/'CorBo_vNext/annotations/morphology.tsv'; CONLLU=ROOT/'CorBo/Corpus_Files/Bororo_UD_enriched_v5_plus_scripture.conllu'; OUT=ROOT/'docs/data'
...
def main():
 ...
 bib=bible_units();allu=coq+hm+bib;validate(allu);OUT.mkdir(parents=True,exist_ok=True)
 outputs={'coqueiro-units.json':coq,'historia-mitica-units.json':hm,'biblia-units.json':bib,'corbo-units.json':allu}
