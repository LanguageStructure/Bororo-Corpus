...
def bakarudoge_units():
 ...
 out=[]
 for d in docs:
  lines=[x.strip() for x in (d.get('documentary_content') or '').splitlines() if x.strip()]
  mode='source';n=0
  for line in lines:
   low=line.casefold()
   if low.startswith(('vocabulário','vocabulario','dados culturais','notas culturais')):break
   if low.startswith(('interpretação','interpretacao','versão portuguesa','versao portuguesa')):mode='translation';continue
   if mode!='source':continue
   if line.casefold().startswith(('nota:','notas:')):continue
   n+=1;uid=f"{d['id'].replace('-BAK-','-BKD-')}-{n:03d}"
   m=ed.get(uid,{})
   out.append({'id':uid,'myth_number':d.get('text_number',''),'title':d.get('title',''),'source_number':'','source':line,'reviewed':m.get('reviewed',''),'linear_portuguese':m.get('linear_portuguese',''),'documentary_translation':'','editorial_note':m.get('editorial_note','')})
 return out
...
class Handler(BaseHTTPRequestHandler):
 ...
 def do_GET(self):
  ...
  if p=='/api/units':
   try:
    c=self.corpus();self.js(morph_rows() if c=='morph' else bakarudoge_units() if c=='bakarudoge' else read_rows(c))
