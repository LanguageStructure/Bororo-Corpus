  ...
  if p=='/api/save':
   try:
    ...
    if c in ('bibliaoral','paulinho'):
     ...
     m[0]['reviewed']=str(body.get('reviewed','')).strip();m[0]['portuguese_reviewed']=str(body.get('portuguese_reviewed','')).strip();write_rows(c,rows);self.js({'ok':True,'id':uid});return
    if c=='bakarudoge':
