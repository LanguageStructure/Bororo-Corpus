...
FILES={'coqueiro':ROOT/'CorBo_vNext/texts/coqueiro/coqueiro_parallel.tsv','hm':ROOT/'CorBo_vNext/texts/historia-mitica/historia_mitica_collation.tsv','adugo':ROOT/'CorBo_vNext/texts/adugo-biri/adugo_biri_parallel.tsv','boeero':ROOT/'CorBo_vNext/texts/boe-ero/boe_ero_parallel.tsv','bakarudoge':ROOT/'CorBo_vNext/texts/bakarudoge/bakarudoge_documentary.tsv','bibliaoral':ROOT/'CorBo_vNext/texts/biblia-oral/biblia_oral_interlinear.tsv','paulinho':ROOT/'CorBo_vNext/texts/paulinho/paulinho_interlinear.tsv','oieigo':ROOT/'CorBo_vNext/texts/oieigo/mito-explicacao.tsv','oieigocanto':ROOT/'CorBo_vNext/texts/oieigo/canto-no-mito.tsv','oieigomeremere':ROOT/'CorBo_vNext/texts/oieigo/o-meremere.tsv','oieigoantonio':ROOT/'CorBo_vNext/texts/oieigo/antonio-caio-g96.tsv','oieigodeus':ROOT/'CorBo_vNext/texts/oieigo/para-deus-garcas.tsv','oieigorodolfo':ROOT/'CorBo_vNext/texts/oieigo/p-rodolfo-m85.tsv','oieigobentinho':ROOT/'CorBo_vNext/texts/oieigo/finado-bentinho-g86.tsv','etnobotanica':ROOT/'CorBo_vNext/texts/etnobotanica/etnobotanica.tsv','morph':ROOT/'CorBo_vNext/annotations/morphology.tsv'}
OIEIGO_COMMENT=ROOT/'CorBo_vNext/texts/oieigo/comment.txt'
OIEIGO_MANIFEST=ROOT/'CorBo_vNext/texts/oieigo/manifest.tsv'
BAK_EDITS=ROOT/'CorBo_vNext/texts/bakarudoge/bakarudoge_linear_edits.tsv'
BM_ROOT=ROOT/'CorBo_vNext/texts/bakaru-maiwu'
...
def bakaru_maiwu_editor_units():
 out=[]
 for path in sorted(BM_ROOT.glob('*/*.tsv')):
  for r in read_tsv_path(path):
   uid=(r.get('id') or '').strip()
   if uid and '-DOC-' not in uid:out.append({**r,'_file':str(path.relative_to(ROOT))})
 return out
def save_bakaru_maiwu(body):
 uid=str(body.get('id','')).strip();rel=str(body.get('_file','')).strip();p=ROOT/rel
 rows=read_tsv_path(p);m=next((r for r in rows if (r.get('id') or '').strip()==uid),None)
 if not m:raise ValueError('Unidade Bakaru Maiwu não encontrada')
 m['reviewed']=str(body.get('reviewed','')).strip()
 fields=list(rows[0].keys())
 if 'reviewed' not in fields:fields.append('reviewed')
...
class Handler(BaseHTTPRequestHandler):
 ...
 def do_GET(self):
  ...
  if p=='/api/oieigo-meta':
   key=parse_qs(urlparse(self.path).query).get('corpus',[''])[0];mapping={'oieigo':'mito-explicacao','oieigocanto':'canto-no-mito','oieigomeremere':'o-meremere','oieigoantonio':'antonio-caio-g96','oieigodeus':'para-deus-garcas','oieigorodolfo':'p-rodolfo-m85','oieigobentinho':'finado-bentinho-g86'};row={}
   if key in mapping and OIEIGO_MANIFEST.exists():row=next((r for r in read_tsv_path(OIEIGO_MANIFEST) if r.get('slug')==mapping[key]),{})
   self.js(row);return
  if p=='/api/comment':
   self.js({'comment':OIEIGO_COMMENT.read_text(encoding='utf-8') if OIEIGO_COMMENT.exists() else ''});return
  if p=='/api/units':
   try:
    c=self.corpus();self.js(morph_rows() if c=='morph' else bakarudoge_units() if c=='bakarudoge' else bakaru_maiwu_editor_units() if c=='bakaru-maiwu' else bible_editor_units(c) if c in BIBLES else read_rows(c))
 ...
 def do_POST(self):
  ...
  if p=='/api/save':
   try:
    ...
    if c=='bakaru-maiwu':
     save_bakaru_maiwu(body);build_indexes();self.js({'ok':True,'id':uid,'rebuilt':True});return
    if c in BIBLES:
    ...
    if c in ('bibliaoral','paulinho','oieigo','oieigocanto','oieigomeremere','oieigoantonio','oieigodeus','oieigorodolfo','oieigobentinho'):
    ...
    if len(m)!=1:raise ValueError('ID estável não encontrado ou duplicado')
    if c=='etnobotanica':m[0]['description']=str(body.get('description','')).strip()
    elif c=='hm':m[0]['reviewed']=str(body.get('reviewed','')).strip()
